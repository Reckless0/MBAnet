...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # self.proj = nn.Conv2d(in_chans, int(embed_dim/2), kernel_size=patch_size, stride=patch_size)
...
class VisionTransformer(nn.Module):
    ...
    # def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
    #              num_heads=12, dim_head=64, mlp_dim=768*4, qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
    #              drop_path_rate=0., norm_layer=nn.LayerNorm, **kwargs):
        super().__init__()
        ...
        ### vit_tiny meta proj layer
        # self.meta_proj = nn.Linear(4, 384) # small
        self.meta_proj = nn.Linear(4, 768)  # base
        # self.meta_proj = nn.Linear(4, 32)  # concat
        self.sigmoid = nn.Sigmoid()

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.pos_embed12 = nn.Parameter(torch.zeros(1, 1 + num_patches + num_patches, embed_dim))
        ...
        trunc_normal_(self.pos_embed12, std=.02)
    ...
    def forward(self, x, meta):
        ...
        if self.is_memory_efficient:
            ...
            for start_idx, (x1,x2) in enumerate(x):
                # _out = self.forward_features(_x, meta)
                _out = self.forward_features(x1, x2, meta)
                _out = self.fc(_out)
                if start_idx == 0:
                    output = _out
                else:
            ...
            return output # just return feature

    # def forward_features(self, x):
    # def forward_features(self, x, meta):
    def forward_features(self, x1, x2, meta):
        # B = x.shape[0]
        B = x1.shape[0]
        ...
        # # channel attention
        # meta = torch.unsqueeze(self.sigmoid(self.meta_proj(meta)), dim=1)
        # x = x * meta
        # # concat
        # meta = self.meta_proj(meta)
        # meta = torch.unsqueeze(meta, dim=1)
        # meta = self.meta_proj(meta).unsqueeze(1)
        # C = meta.shape[2]
        # meta = meta.expand(x.shape[0], x.shape[1], C)
        #
        # x = torch.concat((x, meta),dim=2)

        cls_tokens = self.cls_token.expand(B, -1, -1)

        x12 = torch.cat((cls_tokens, x12), dim=1)

        # # concat 12
        # x = torch.cat((x1, x2), dim=1)

        pos_embed = self.interpolate_pos_encoding(x12, self.pos_embed12)
        x = x12 + pos_embed
    ...
    def forward_selfattention(self, x):
        ...
        class_pos_embed = self.pos_embed12[:, 0]
        patch_pos_embed = self.pos_embed12[:, 1:]
    ...
    def forward_return_n_last_blocks(self, x, n=1, return_patch_avgpool=False):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        pos_embed = self.interpolate_pos_encoding(x, self.pos_embed12)
        x = x + pos_embed
        x = self.pos_drop(x)
...
def deit_tiny(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    model = VisionTransformer(img_size=img_size, num_classes=2, patch_size=patch_size, embed_dim=192,
                              depth=12, num_heads=3, dim_head=64, mlpatio=4,
...
def deit_small(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    model = VisionTransformer(img_size=img_size, num_classes=2,
